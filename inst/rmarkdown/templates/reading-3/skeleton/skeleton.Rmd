...
We can verify that the graph is much smallerr, since it includes only major highways:
```{r}
hamilton_motorways
```
Then, we use this  creates a buffer around motorways. The resulting object, which we call `motorway_buffers` is no longer a graph, but a simple features object:
```{r}
...
```{r}
ggplot() + # Create blank ggplot object
  geom_sf(data = hamilton_sustenance, # To plot a `sf` object we use `geom_sf()`, and for this plot we want to use `hamilton_eateries`
