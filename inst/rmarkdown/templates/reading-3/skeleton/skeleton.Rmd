...
hamilton_motorways
```
Then, we use this chuck of codes that creates a buffer around motorways. The resulting object, which we call `motorway_buffers` is no longer a graph, but a simple features object:
```{r}
...
We see that the nearest nodes to our sample addresses are nodes 100 (origin) and157 (destination).
